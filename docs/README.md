...
- The [foundation example](../examples/foundation/index.html) is the current
...
The site will support three views over the same sources:
...
It will use PWF's own shell, search adapter, themes, content layouts, and
progressive navigation. Ordinary URLs and server-rendered pages remain the
baseline, so every documentation page can be linked, indexed, printed, and read
